...
import {ALLASSIGNMENT_QUERY, ALLCOURSE_NAME_QUERY} from "../../../utils/graphql-query";
...
export default function AssignmentNewEditForm({ isEdit, assignment }) {
    ...
    const handleDrop = useCallback(
        async (acceptedFiles) => {
            ...
            setValue(
                'attachments',
                [ ...getValues('attachments'), ...base64Files]
            );
        },
        [setValue]
    );
        ...
        <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
            <Grid container spacing={3}>
                <Grid item xs={12} md={8}>
                    <Card sx={{ p: 3 }}>
                        <Stack spacing={3}>
                            <RHFTextField disabled={isEdit} name="name" label="Assignment Name" />
                ...
                <Grid item xs={12} md={4}>
                    <Stack spacing={3}>
                        <Card sx={{ p: 3 }}>
                            <Stack spacing={3}>
                                <RHFSelect disabled={isEdit} name="course" label="Course">
                                    {course.map((option) => (
                                ...
                                <RHFDatePicker name="dueDate" label="Due Date" minDate={Date.now()}/>
                            </Stack>
                        </Card>
