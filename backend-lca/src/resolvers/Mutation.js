...
async function logIn(parent, args, context, info) {
    ...
    const validPassword = await bcrypt.compare(args.userInput.password, user.password);
...
async function refreshJWT(parent, args, context, info) {
    ...
    if (!refreshJWTToken) throw new Error('No refresh token found! Please login!')
    ...
    if (user === null) throw new Error('Refresh token expired or does not exists! Please login!')
...
async function changePassword(parent, args, context, info) {
    ...
    const validPassword = await bcrypt.compare(args.oldPassword, userPassword.password);
    if (!validPassword) throw new Error("Invalid password.");
...
async function upsertUser(parent, args, context, info) {
    const {id, username, firstName, lastName, email, phone, about, roleId} = args.userInput;
    ...
    if (id == 0) {
        ...
        if (!verifyUser(context.userId, id) && !verifyRole(context.roleId, process.env.ROLE_ADMIN_CODE)) {
...
async function upsertCourse(parent, args, context, info) {
    const {id, name, description, startDate, endDate, instructedBy} = args.courseInput;
    ...
    if (!verifyRole(instructorRole.role.id, process.env.ROLE_TEACHER_CODE)) {
        throw Error("User is not teacher!");
    }
...
async function registerCourse(parent, args, context, info) {
    ...
    if (student.length !== studentId.length) throw Error("Some students are not found.");
        ...
        groups.forEach(group => {
            group.studentGroup.forEach(itemStudentGroup => {
                if (studentId.includes(itemStudentGroup.student.id)) {
                    throw Error(`${itemStudentGroup.student.firstName} ${itemStudentGroup.student.lastName} registered for other group.`);
                }
            })
        })
    }
...
async function upsertAssignment(parent, args, context, info) {
    const {id, name, description, courseId, dueDate, attachments} = args.assignmentInput;
    ...
    if (!verifyUser(context.userId, courseInstructor.instructor.id) && !verifyRole(context.roleId, process.env.ROLE_ADMIN_CODE))
        throw Error("User does not have permission.");
    ...
    const assignmentAttachments = await context.prisma.assignmentAttachment.createMany({
        data: item
    });
...
async function upsertAnalyzer(parent, args, context, info) {
    const {id, name, description, analyzerBase64, analyzerFileName, analyzerFileExtension, analyzerFileSize} = args.analyzerInput;
